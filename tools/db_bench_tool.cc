  ...
  void DoWrite(ThreadState* thread, WriteMode write_mode) {
    ...
    while (!duration.Done(entries_per_batch_)) {
      ...
      if (t_cur_time - t_last_time > 10*1e6) {
        ...
        RECORD_INFO(1,"now=,%.2f,s speed=,%.2f,MB/s,%.1f,iops size=,%.1f,MB average=,%.2f,MB/s,%.1f,iops compaction:,%ld,pick_time:,%.3f s, IO_delete_key_nums:,%ld\n",
          ...
          1.0*global_stats.pick_compaction_time*1e-6,global_stats.IO_delete_key_nums);
  ...
  void YCSBWorkloadE(ThreadState* thread) {
    ...
    while (!duration.Done(1)) {
      ...
      if (next_op < 95){
        ...
        int64_t i = 0;
        int64_t bytes = 0;
        for (iter->Seek(key); i < scan_length && iter->Valid(); iter->Next()) {
