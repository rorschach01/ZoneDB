...
SstableMetadata::SstableMetadata(const InternalKeyComparator* icmp, const NvmCfOptions* nvmcfoption)
    ...
    //mu_ = new MyMutex();
    rwlock = new ZoneMutex();
    RECORD_LOG("create SstableMetadata\n");
...
// 从后往前检索 compection_files 的状态
int SstableMetadata::UpdateCompactionState(std::vector<FileMetaData*>& L0files, int compaction_files_num, int waitbanlanced){
    ...
    for(;file_num >= 0; file_num--, compaction_files_num--){  //目前所有table加入compaction_files，后面可设置数量
        if(compaction_files_num<=0) break;
        ...
        //文件标志为待compaction
        FileEntry* tmp = FindFile(L0files[file_num]->fd.GetNumber());
        tmp->compacting = true;
        //防止L0全部刷写下去了
        if (waitbanlanced && compaction_files_size >= Level0_column_compaction_limited_size) {
            break;
        }
    }
