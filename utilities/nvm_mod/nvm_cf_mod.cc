...
NvmCfModule::NvmCfModule(NvmCfOptions* nvmcfoption, const std::string& cf_name,
  ...
  nvm_dir_no_exists_and_create(nvmcfoption_->pmem_path);
  char buf[100];
  ...
  //预分割采样 10000 key
  nvmcfoption_->zones_num = 32;
  trie = new Trie(10000, nvmcfoption_->zones_num);
}
...
bool NvmCfModule::AddL0TableRoom(uint64_t filenum, char** raw,
  ...
  if (index == -1 || tmp == nullptr) {
    printf("error:AddL0TableRoom AllocSstable error!index:%d\n", index);
    return false;
  }
  *raw = tmp;
...
ColumnCompactionItem* NvmCfModule::PickColumnCompaction(VersionStorageInfo* vstorage){
  ...
  // compaction 文件数目
  int compaction_files_num = 0;
  if(L1files.size() == 0){
    compaction_files_num = nvmcfoption_->Column_compaction_no_L1_select_L0;  //L0 无文件
  }else{
    compaction_files_num = nvmcfoption_->Column_compaction_have_L1_select_L0;
  }

  //UpdateCompactionState(L0files); //更新compaction files
  int waitbanlanced = getwait();
  UpdateCompactionState(L0files, compaction_files_num, waitbanlanced);
...
ColumnCompactionItem* NvmCfModule::PickZoneCompaction(VersionStorageInfo* vstorage){
  ColumnCompactionItem* c = nullptr;
  //todo:选择数据
  auto L0files = vstorage->LevelFiles(0);
  auto L1files = vstorage->LevelFiles(1);
  ...
  int waitbanlanced = getwait();
  if(UpdateCompactionState(L0files, compaction_files_num, waitbanlanced)){//更新compaction files
    updatestate++;
    if(updatestate == 2){//L1各个分区都存在文件
      nvmcfoption_->Level0_compaction_limited_size = 128ul * 1024 * 1024; //限制L0合并参与数据量
    }
  }
  uint64_t comfiles_num = sst_meta_->compaction_files.size(); //compaction files number
  if(comfiles_num == 0){
    ...
    RECORD_LOG("comfiles_num:%d, l0:%ld\n",comfiles_num, L0files.size());
  }
  ...
  for(unsigned int i=0;i < comfiles_num; i++) {
    ...
    // 注意这里的判断逻辑是因为 compaction_files 和 L0files 的逻辑顺序相同
    if(find_file){
      first_key_indexs.push_back(L0files.at(j)->first_key_index);  //对应文件的first_key_index插入
      j++;
    }
  ...
  // 初始化 keys_num 和 keys_size，初始化迭代器信息
  for(unsigned int i=0;i < comfiles_num; i++) {
    keys_num[i] = 0;
    keys_size[i] = 0;
  }
  ...
  c = new ColumnCompactionItem();
  uint64_t all_compaction_size = 0;
  auto user_comparator = icmp_->user_comparator(); //比较只根据user key比较
  // 初始化合并迭代器
  KeysMergeIterator* k_iter = new KeysMergeIterator(&comfiles,&first_key_indexs,user_comparator);
  ...
  //拿到 minsmallest 和 maxlargest 的范围 Key 值
  InternalKey minsmallest; //  smallest <= comfiles  <= largest       
  InternalKey maxlargest;
  ...
  // 设定 compaction 的区间
  InternalKey min_zone_key;      
  InternalKey max_zone_key;
  for(unsigned int i = 0; i < trie->zones_key.size(); i++){
    if(user_comparator->Compare(ExtractUserKey(minsmallest.Encode()),ExtractUserKey(trie->zones_key[i].Encode())) < 0){
      min_zone_key = minsmallest;
      max_zone_key = trie->zones_key[i];
      break;
    }
    // 最后一个区间
    if(i == trie->zones_key.size()-1){
      min_zone_key = trie->zones_key[i];
      max_zone_key = maxlargest;
    }
  }
  RECORD_LOG("L0 min_zone_key:%s max_zone_key:%s\n",min_zone_key.DebugString(true).c_str(),max_zone_key.DebugString(true).c_str());
  ...
  while(k_iter->Valid()){
    //遍历到区间边界
    if(user_comparator->Compare(ExtractUserKey(comfiles[files_index]->keys_meta[keys_index].key.Encode())
      ,ExtractUserKey(max_zone_key.Encode())) >= 0 ){
    //if(all_compaction_size > nvmcfoption_->Level0_compaction_limited_size){
      int next_files_index,next_keys_index;
      k_iter->GetCurret(next_files_index,next_keys_index);
      if(user_comparator->Compare(ExtractUserKey(comfiles[files_index]->keys_meta[keys_index].key.Encode()),ExtractUserKey(comfiles[next_files_index]->keys_meta[next_keys_index].key.Encode())) == 0){
        //如果与下一个key相等，则继续加入compaction,防止出现 SST 只重复一个 key 的情况
        k_iter->GetCurret(files_index,keys_index);
        itemsize = comfiles.at(files_index)->keys_meta[keys_index].size;
        keys_num[files_index]++;
        keys_size[files_index] += itemsize;
        all_compaction_size += itemsize;
        k_iter->Next();
        continue;
      }
  ...
  //k_iter 遍历完所有文件到 null 指针了（特殊情况）
  if(all_compaction_size > 0 && !k_iter->Valid()){
    c->L0largest = comfiles.at(files_index)->keys_meta[keys_index].key;
  }
  //写入需要 compaction 的文件信息
  for(unsigned int index = 0;index < comfiles_num;index++){
    if(keys_num[index] != 0){
      tmp = comfiles.at(index);
      c->files.push_back(tmp);
  ...
  c->L0select_size = all_compaction_size;
  // 填入 version_set 中的 FileMeta 信息
  FileMetaData* ftmp = nullptr;
  uint64_t filenum = 0;
  L0files = vstorage->LevelFiles(0);
  for(unsigned int i = 0;i < c->files.size();i++){
  ...
  //拿到 L1 对应的待 compaction 的文件
  L1files = vstorage->LevelFiles(1);
  for(unsigned int i = 0;i < L1files.size();i++){
    RECORD_LOG("L1table:%lu [%s-%s]\n",L1files.at(i)->fd.GetNumber(),L1files.at(i)->smallest.DebugString(true).c_str(),L1files.at(i)->largest.DebugString(true).c_str());
  }
  std::vector<FileMetaData*> L1overlapfiles;
  vstorage->GetOverlappingInputs(1, &c->L0smallest, &c->L0largest, &L1overlapfiles);
  //vstorage->GetOverlappingInputs(1,&minsmallest,&maxlargest,&L1overlapfiles);
  for(unsigned int i = 0;i < L1overlapfiles.size();i++){
    RECORD_LOG("L1overtable:%lu [%s-%s]\n",L1overlapfiles.at(i)->fd.GetNumber(),L1overlapfiles.at(i)->smallest.DebugString(true).c_str(),L1overlapfiles.at(i)->largest.DebugString(true).c_str());
  }

  //填充 L1 compaction 的文件
  for(unsigned int i = 0;i < L1overlapfiles.size(); i++){
      c->L1compactionfiles.push_back(L1overlapfiles.at(i));
  }
...
double NvmCfModule::GetCompactionScore(){
  ...
  score = 1.0 + (double)compactionfilenum/32;
  return score;
}
...
bool NvmCfModule::Get_hash(VersionStorageInfo* vstorage,Status *s,const LookupKey &lkey,std::string *value){
  auto L0files = vstorage->LevelFiles(0);
  ...
  for(unsigned int i = 0;i < findfiles.size();i++){
    file = findfiles.at(i);
    std::unordered_map<std::string,int>::const_iterator iter = file->key_hash_index.find(user_key.ToString(true));
    if(iter != file->key_hash_index.end() && user_comparator->Compare(user_key,ExtractUserKey(file->keys_meta[iter->second].key.Encode())) == 0){
      GetValueInFile(file,iter->second,value);
      *s=Status::OK();
      return true;    
    }
  }
...
int NvmCfModule::UserKeyCompare(Slice *user_key,InternalKey *t_key){
  auto user_comparator = icmp_->user_comparator();  
  if(user_comparator->Compare(*user_key,t_key->user_key()) == 0) {
    return 1;
  }
  else {
    return -1;
