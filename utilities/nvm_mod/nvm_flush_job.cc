...
Status NvmFlushJob::WriteLevel0Table() {
  ...
  Status s;
  {
...

    {
      ...
      /*s = BuildTable(
          ...
          mutable_cf_options_.paranoid_file_checks, cfd_->internal
          _stats(),
  ...
  if (s.ok() && meta_.fd.GetFileSize() > 0) {
    ...
    // that key range.
    // Add file to L0
    // edit 参数添加了 meta_.is_level0 和 meta_.first_key_index
 ...
 // TableProperties tp;
  ...
  if (iter->Valid() || !range_del_agg->IsEmpty()) {
    ...
    for (; c_iter.Valid(); c_iter.Next()) {
      ...
      /*if(number_entry < 5){
        std::string ss(key.ToString(true));
        std::string iss(ikey.DebugString(true));
        RECORD_LOG("L0TableBuilder::Add_Slice key_size:%d key:%s ikey_size:%d ikey:%s\n",
            ss.size(), ss, ikey.user_key.size() + 8, iss);
        auto sss = ss.c_str();
        for(size_t kk =0 ; kk < ss.size(); kk++){
          RECORD_LOG("key[%d]:%c ", kk, sss[kk]);
        }
        RECORD_LOG("\n");
      }*/
